...
import { machines, globalRanking, tickerItems, prizes } from '../data/mock';
    ...
    <div className="overflow-x-hidden">
      ...
      <section className="relative min-h-[90svh] flex items-center justify-center bg-dots overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center py-20">
          ...
          <h1 className="font-game font-black text-white leading-tight tracking-tight mb-6">
            <span
              className="block text-4xl sm:text-6xl lg:text-7xl glow-purple"
              style={{ textShadow: '0 0 40px rgba(168,85,247,0.6), 0 0 80px rgba(168,85,247,0.3)' }}
            >
              JOGUE.
            </span>
            <span
              className="block text-4xl sm:text-6xl lg:text-7xl glow-blue mt-1"
              style={{ textShadow: '0 0 40px rgba(96,165,250,0.6), 0 0 80px rgba(96,165,250,0.3)' }}
            >
              VENÇA.
            </span>
            <span
              className="block text-4xl sm:text-6xl lg:text-7xl mt-1"
              style={{
          ...
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            ...
            <Link
              to="/ranking"
              className="inline-flex items-center justify-center gap-2 px-8 py-4 rounded-xl text-base font-semibold text-purple-300 btn-outline"
            >
              <Trophy size={18} />
              Ver Ranking
            </Link>
          ...
          {grandPrize && (
            <div className="mt-12 inline-flex items-center gap-3 px-6 py-3 rounded-2xl card-neon-gold">
              ...
              <span className="text-sm text-slate-300">
                Grand Prize de{' '}
                <strong className="text-gradient-gold text-base font-black">R$ {grandPrize.amount.toLocaleString('pt-BR')}</strong>
                {' '}ainda disponível — Abril 2026
              </span>
            </div>
          )}
        </div>
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          <div className="flex items-center justify-between mb-10">
            <div>
              ...
              <h2 className="font-game font-bold text-white text-2xl sm:text-3xl">
                Top Jogadores
              </h2>
            </div>
            <Link
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {top3.map((entry, i) => {
              ...
              return (
                <div
                  key={entry.nickname}
                  className={`relative rounded-2xl border bg-gradient-to-b p-6 text-center card-neon transition-all duration-200 ${glows[i]}`}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {activeMachines.slice(0, 6).map((machine) => (
              <Link
                key={machine.id}
                to="/mapa"
                className="rounded-2xl p-5 card-neon card-neon-blue transition-all duration-200 hover:-translate-y-1 group"
              >
      ...
      <section className="py-20 bg-[#0a0018]">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 text-center">
          ...
          <h2 className="font-game font-bold text-white text-2xl sm:text-3xl mb-12">
            Como Funciona
          </h2>
            ...
            {[
              ...
              <div key={i} className="flex flex-col items-center gap-3">
                <div className="relative">
                  <div className="w-16 h-16 rounded-2xl card-neon flex items-center justify-center text-3xl animate-float"
                    style={{ animationDelay: `${i * 0.3}s` }}>
                    {step.emoji}
                  </div>
                  {i < 3 && (
                    <div className="hidden sm:block absolute top-1/2 -right-7 w-6 h-px bg-gradient-to-r from-purple-500/50 to-transparent" />
                  )}
                </div>
          ...
          <Link
            to="/como-funciona"
            className="inline-flex items-center gap-2 px-6 py-3 rounded-xl text-sm font-semibold text-purple-300 btn-outline"
          >
      ...
      <section className="py-20 bg-dots relative overflow-hidden">
        <div
          ...
          style={{ background: 'radial-gradient(ellipse 60% 50% at 50% 50%, rgba(124,58,237,0.12) 0%, transparent 100%)' }}
        />
        <div className="relative max-w-2xl mx-auto px-4 sm:px-6 text-center">
          ...
          <p className="text-slate-300 text-lg mb-3">
            O maior score global de abril leva
          </p>
          <p
            className="font-game font-black text-5xl sm:text-6xl mb-2 animate-neon-pulse"
            style={{
              background: 'linear-gradient(135deg, #fbbf24, #f59e0b)',
              WebkitBackgroundClip: 'text',
              WebkitTextFillColor: 'transparent',
          ...
          <p className="text-slate-500 text-sm mb-10">em dinheiro • Encerra 30/04 às 23h59</p>
